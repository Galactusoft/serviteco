...
@Component({
    selector: 'reporte-adquisicion-equipos',
    templateUrl: './reporte-adquisicion-equipos.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteAdquisicionEquiposComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getAdquisicionEquiposImportadorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        const busqueda = this.form.getRawValue();
        this._gestionReporteService.getExportAdquisicionEquiposImportadorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Referencia', 'Marca', 'Cantidad', 'Distribuidor'];
                var footer = [];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_adquisición_equipos_distribuidor_importador', head, footer);
            } else {
                Swal.fire({
