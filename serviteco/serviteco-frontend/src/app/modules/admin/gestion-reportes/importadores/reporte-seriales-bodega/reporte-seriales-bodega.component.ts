...
@Component({
    selector: 'reporte-seriales-bodega',
    templateUrl: './reporte-seriales-bodega.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteSerialesBodegaComponent implements OnInit, OnDestroy {
    ...
    recentTransactionsTableColumns: string[] = ['serial', 'referencia', 'marca', 'categoria'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getSerialesBodegaImportadorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        const busqueda = this.form.getRawValue();
        this._gestionReporteService.getExportSerialesBodegaImportadorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Serial', 'Referencia', 'Marca', 'Categoría'];
                var footer = [];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_seriales_en_bodega_importador', head, footer);
            } else {
                Swal.fire({
