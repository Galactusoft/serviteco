...
@Component({
    selector: 'reporte-usuarios-activos',
    templateUrl: './reporte-usuarios-activos.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteUsuariosActivosComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getUsuariosActivosImportadorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportUsuariosActivosImportadorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Usuario', 'Nombre Apellidos', 'Identificación', 'Correo', 'Teléfono', 'Cargo','Estado','Fecha de sistema'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_usuarios_activos_por_importador', head, []);
            } else {
                Swal.fire({
