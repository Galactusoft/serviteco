...
@Component({
    selector: 'reporte-mano-obra-costos',
    templateUrl: './reporte-mano-obra-costos.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteManoObraCostosComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getManoObraCostosImportadorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportManoObraCostosImportadorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Ticket', 'Mano Obra', 'Referencia', 'Categoría', 'Marca', 'Distribuidor', 'Taller', 'Cantidad', 'Valor Unitario', 'Total'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_mano_obra_costos_por_importador', head, []);
            } else {
                Swal.fire({
