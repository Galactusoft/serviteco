...
@Component({
    selector: 'reporte-referencias-mas-garantias',
    templateUrl: './reporte-referencias-mas-garantias.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteReferenciasMasGarantiasComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getReferenciasMasGarantiasPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportReferenciasMasGarantiasPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Referencia', 'Descripción', 'Marca', 'Categoría', 'Distribuidor', 'Cantidad garantías'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_referencias_mas_garantias_importador', head, []);
            } else {
                Swal.fire({
