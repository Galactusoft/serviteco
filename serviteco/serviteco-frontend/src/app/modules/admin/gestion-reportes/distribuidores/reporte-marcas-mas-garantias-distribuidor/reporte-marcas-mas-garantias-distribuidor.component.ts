...
@Component({
    selector: 'reporte-marcas-mas-garantias-distribuidor',
    templateUrl: './reporte-marcas-mas-garantias-distribuidor.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteMarcasMasGarantiasDistribuidorComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['nombre', 'descripcion', 'categoria', 'distribuidor', 'cantidad'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getMarcasMasGarantiasDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportMarcasMasGarantiasDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Marca', 'Descripción', 'Categoría', 'Distribuidor', 'Cantidad garantías'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_marcas_mas_garantias_distribuidor', head, []);
            } else {
                Swal.fire({
