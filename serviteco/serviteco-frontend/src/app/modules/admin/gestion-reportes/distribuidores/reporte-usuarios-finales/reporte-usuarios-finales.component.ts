...
@Component({
    selector: 'reporte-usuarios-finales',
    templateUrl: './reporte-usuarios-finales.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteUsuariosFinalesComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['nombres', 'apellidos', 'identificacion', 'correo', 'telefono', 'direccion'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getUsuariosFinalesDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportUsuariosFinalesDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Nombres', 'Apellidos', 'Identificación', 'Correo', 'Teléfono', 'Dirección'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_usuarios_finales_distribuidor', head, []);
            } else {
                Swal.fire({
