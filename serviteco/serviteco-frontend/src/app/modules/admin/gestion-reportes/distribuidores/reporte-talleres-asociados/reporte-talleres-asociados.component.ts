...
@Component({
    selector: 'reporte-talleres-asociados',
    templateUrl: './reporte-talleres-asociados.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteTalleresAsociadosComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getTalleresAsociadosDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportTalleresAsociadosDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Id', 'Nombre', 'Dirección', 'Teléfono', 'Correo', 'Correo contabilidad', 'Estado', 'Fecha Sistema'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_talleres_asociados_distribuidor', head, []);
            } else {
                Swal.fire({
