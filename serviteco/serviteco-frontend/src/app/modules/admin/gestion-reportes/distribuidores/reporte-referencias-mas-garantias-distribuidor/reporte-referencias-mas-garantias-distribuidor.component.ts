...
@Component({
    selector: 'reporte-referencias-mas-garantias-distribuidor',
    templateUrl: './reporte-referencias-mas-garantias-distribuidor.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteReferenciasMasGarantiasDistribuidorComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['nombre', 'descripcion', 'marca', 'categoria', 'distribuidor', 'cantidad'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getReferenciasMasGarantiasDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportReferenciasMasGarantiasDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Referencia', 'Descripción', 'Marca', 'Categoría', 'Distribuidor', 'Cantidad garantías'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_referencias_mas_garantias_distribuidor', head, []);
            } else {
                Swal.fire({
