...
@Component({
    selector: 'reporte-aprobacion-rechazo',
    templateUrl: './reporte-aprobacion-rechazo.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteAprobacionRechazoComponent implements OnInit, OnDestroy {
    ...
    tableColumnsTalleres: string[] = ['ticket', 'nombre_completo', 'identificacion', 'telefono', 'correo', 'ciudad', 'fecha_ingreso', 'descripcion_falla', 'diagnostico_falla',
        'observacion_diagnostico', 'observacion_respuesta', 'creado_por', 'estado_actual', 'distribuidor'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getAprobacionRechazoSolicitudesDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportAprobacionRechazoSolicitudesDistribuidorPaginator(busqueda).subscribe(data => {
            if (this.dataCount > 0 || this.dataCountTalleres > 0 ) {
                var head = ['ticket', 'nombre_completo', 'identificacion', 'telefono', 'correo', 'ciudad', 'fecha_ingreso', 'descripcion_falla', 'diagnostico_falla',
                    'observacion_diagnostico', 'observacion_respuesta', 'creado_por', 'estado_actual', 'distribuidor'];
                var headTaller = ['ticket', 'nombre_completo', 'identificacion', 'telefono', 'correo', 'ciudad', 'fecha_ingreso', 'descripcion_falla', 'diagnostico_falla',
                    'observacion_diagnostico', 'observacion_respuesta', 'creado_por', 'estado_actual', 'distribuidor'];
                this.exportReportAsExcelFile(data.registros, 'reporte_aprobacion-rechazo-solicitudes-distribuidor', head, data.talleres, headTaller);
            } else {
                Swal.fire({
    ...
    exportReportAsExcelFile(obj: any[], excelFileName: string, customHead: string[], objTaller: any[], customHeadTaller: string[]): void {
        ...
        const workbook: XLSX.WorkBook = { Sheets: { 'aprobadas': worksheet, 'rechazadas': worksheetTaller }, SheetNames: ['aprobadas', 'rechazadas'] };
