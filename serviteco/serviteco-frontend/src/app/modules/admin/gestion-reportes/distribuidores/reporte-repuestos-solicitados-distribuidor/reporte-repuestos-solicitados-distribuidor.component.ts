...
@Component({
    selector: 'reporte-repuestos-solicitados-distribuidor',
    templateUrl: './reporte-repuestos-solicitados-distribuidor.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteRepuestosSolicitadosDistribuidorComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['ticket', 'repuesto', 'material', 'pieza_fabricante', 'referencia', 'categoria', 'marca', 'distribuidor', 'taller', 'cantidad', 'valor_unitario', 'total'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getRepuestosSolicitadosDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportRepuestosSolicitadosDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Ticket', 'Repuesto', 'Material', 'Pieza fabricante', 'Referencia', 'Categoría', 'Marca', 'Distribuidor', 'Taller', 'Cantidad', 'Valor Unitario', 'Total'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_repuestos_mas_solicitados_distribuidor', head, []);
            } else {
                Swal.fire({
