...
@Component({
    selector: 'reporte-usuarios-asociados-distribuidor',
    templateUrl: './reporte-usuarios-asociados-distribuidor.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteUsuariosAsociadosDistribuidorComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['username', 'nombre_completo', 'identificacion', 'correo', 'telefono', 'cargo','estado','fecha_sistema'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getUsuariosAsociadosDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportUsuariosAsociadosDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Usuario', 'Nombre Apellidos', 'Identificación', 'Correo', 'Teléfono', 'Cargo','Estado','Fecha de sistema'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_usuarios_asociados_distribuidor', head, []);
            } else {
                Swal.fire({
