...
@Component({
    selector: 'reporte-solicitud-garantia-distribuidor',
    templateUrl: './reporte-solicitud-garantias-distribuidor.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteSolicitudGarantiasDistribuidorComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getSolicitudGarantiasDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        const busqueda = this.form.getRawValue();
        this._gestionReporteService.getExportSolicitudGarantiasDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['ticket', 'nombre_completo', 'identificacion', 'telefono', 'correo', 'ciudad', 'fecha_ingreso','descripcion_falla','diagnostico_falla',
                            'observacion_diagnostico','observacion_respuesta','creado_por','estado_actual','distribuidor'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_solicitudes_garantia_distribuidor', head, []);
            } else {
                Swal.fire({
