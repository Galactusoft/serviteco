...
@Component({
    selector: 'reporte-tickets-por-cobrar',
    templateUrl: './reporte-tickets-por-cobrar.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReporteTicketsPorCobrarComponent implements OnInit, OnDestroy {
    ...
    tableColumns: string[] = ['ticket', 'mano_obra', 'referencia', 'categoria', 'marca', 'distribuidor', 'taller', 'cantidad', 'valor_unitario', 'total'];
    orderBy: string = "1";
    order: string = "asc";
    filter: string = "all";
    pageIndex: number = 0;
    ...
    */
    ...
    consultarData() {
        ...
        // Search
        this._gestionReporteService.getTicketsPorCobrarDistribuidorPaginator(paginator).subscribe(data => {
    ...
    exportar(): void {
        ...
        this._gestionReporteService.getExportTicketsPorCobrarDistribuidorPaginator(busqueda).subscribe(data => {
            if (data.registros.length > 0) {
                var head = ['Ticket', 'Mano Obra', 'Referencia', 'Categoría', 'Marca', 'Distribuidor', 'Taller', 'Cantidad', 'Valor Unitario', 'Total'];
                this._gestionReportesService.exportReportAsExcelFile(data.registros, 'reporte_tickets_por_cobrar_distribuidor', head, []);
            } else {
                Swal.fire({
